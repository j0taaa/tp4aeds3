...
public class FileManager {
  ...
  public Produto[] conditionalSearch(Predicate<Produto> condition, int max)
    throws Exception {
    raf.seek(0);
    raf.readInt();
  ...
  /**
   * Retorna posição do ponteiro RAF.
   * @return long - posição do ponteiro
   * @throws Exception
   */
  ...
  /**
   * Preenche Hash Extensível com registros (Produtos) do arquivo.
   * @throws Exception
   */
  ...
  /**
   * Preenche Lista Invertida com registros (Produtos) do arquivo.
   * @throws Exception
   */
  public Produto findProdutoUsingHash(HashExtensivel he, int id)
    throws Exception {
    long pos = he.pesquisar(id);
    if (pos == -1) {
      return null;
  ...
  public Produto findProdutoUsingHash(int id) throws Exception {
    return findProdutoUsingHash(he, id);
  }

  /**
   * Busca por registros (Produtos) no arquivo com base em título (parâmetro). Retorna array de Produtos.
   * @param title
   * @param inclusive
   * @return
   * @throws Exception
   */
  public Produto[] findProdutosByTitle(String title, boolean inclusive)
    throws Exception {
  ...
  /**
   * Busca por registros (Produtos) no arquivo com base em categoria (parâmetro). Retorna array de Produtos.
   * @param category
   * @return
   * @throws Exception
   */
  ...
  public Produto[] findProdutosByCategory(String category, int max)
    throws Exception {
    int[] ids = li2.buscar(category);
    Produto[] res = new Produto[max];
    for (int i = 0; i < max && i < ids.length; i++) {
      res[i] = findProdutoUsingHash(he, ids[i]);
    }
    return res;
  }
  ...
  /**
   * Busca produto com base em id (parâmetro) e retorna produto com árvore B.
   * @param id
   * @return
   * @throws Exception
   */
  ...
  /**
   * Busca por registros (Produtos) no arquivo com base em descrição (parâmetro). Retorna array de Produtos que contém a substring.
   * @param subString
   * @return
   * @throws Exception
   */
  public ArrayList<Produto> getProdutosByDescricaoBM(String subString)
    throws Exception {
  ...
  /**
   * Busca produto com base no objeto de produto p e retorna produto com árvore B. Funciona ao colocar a posição do ponteiro no início do arquivo.
   * @param p
   * @return
   * @throws Exception
   */
